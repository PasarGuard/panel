...
def test_host_finalmask_new_types(access_token):
    ...
    finalmask_payload = {
        ...
        "udp": [
            ...
            },
            {
                ...
                "settings": {
                    "reset": "30-60",
                    "noise": [{"type": "str", "packet": "padding", "delay": "10-20"}],
                },
            },
        ],
        "quicParams": {
    ...
    try:
        ...
        noise_item = (fm["udp"][5]["settings"].get("noise") or [{}])[0]
        assert noise_item.get("type") == "str"
        assert noise_item.get("packet") == "padding"
        assert "apply_to" not in noise_item
    finally:
        client.delete(f"/api/host/{host_id}", headers={"Authorization": f"Bearer {access_token}"})
        delete_core(access_token, core["id"])
