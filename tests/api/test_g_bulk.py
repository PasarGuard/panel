...
def test_add_groups_to_users(access_token):
    ...
    response = client.get("/api/v1/groups", headers={"Authorization": f"Bearer {access_token}"})
    group_ids = [g["id"] for g in response.json()["groups"]]

    response = client.post(
        "/api/v1/groups/bulk/add",
        headers={"Authorization": f"Bearer {access_token}"},
    ...
    for username in usernames:
        response = client.get(f"/api/v1/user/{username}", headers={"Authorization": f"Bearer {access_token}"})
...
def test_update_users_expire(access_token):
    ...
    client.put(
        f"/api/v1/user/{usernames[0]}",
        headers={"Authorization": f"Bearer {access_token}"},
        json={"expire": "2025-01-01T00:00:00+00:00"},
    )
    client.put(
        f"/api/v1/user/{usernames[1]}",
        headers={"Authorization": f"Bearer {access_token}"},
        json={"expire": "2026-01-01T00:00:00+00:00"},
    )

    response = client.post(
        "/api/v1/users/bulk/expire",
        headers={"Authorization": f"Bearer {access_token}"},
        json={"amount": 3600},
    )
...
def test_update_users_proxy_settings(access_token):
    ...
    response = client.post(
        "/api/v1/users/bulk/proxy_settings",
        headers={"Authorization": f"Bearer {access_token}"},
