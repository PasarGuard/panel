...
from app.db.crud.client_template import get_client_templates
...
def test_singbox_preserves_explicit_groups_and_reports_empty_generated_group():
    ...
    assert config.config["outbounds"][1]["outbounds"] == ["fixed"]
    assert config.config["outbounds"][2]["outbounds"] == ["server"]
