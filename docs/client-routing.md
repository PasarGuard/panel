# Application routing and server selection
...
When an Xray or Sing-box application needs automatic server groups, select a
server-group generator as its configuration source. Routing, DNS, group settings,
delivery and preview remain in this application workflow. A generator and a
native document are alternative sources for one response; they are not layered
on top of each other.

An assigned configuration opens directly in the editor. The configuration name
appears in the source selector below the application name. **Configuration options** contains
...
| Xray or Happ with automatic groups | Xray generator parameters |
| Sing-box with automatic groups | Sing-box generator parameters |
...
Generator code is explicitly labelled **Generator JSON**. It contains settings
such as `pools`, `health_check`, `dns` and `routing_rules`; it is not an Xray or
Sing-box configuration to import into a client. Preview shows the actual generated
client configuration separately. See [server-group generators](subscription-profiles.md)
for membership, stable routing targets and the differences between the two cores.
...
the draft available. Background refreshes also preserve unapplied field edits and
their original revision. Navigation warns before discarding unsaved changes.
...
An explicit `profile_id` instead selects a server-group generator. It is mutually
exclusive with `template_id`. Managed `happ_routing` cannot be combined with either
full-JSON source. All sources use the same rule order, storage and atomic workspace
operation.

Existing Sing-box subscriptions retain their historical group population behavior.
Only an explicitly assigned native document preserves authored nonempty selector
and urltest lists; missing or empty lists continue to request generated members.
...
document in the same transaction, choosing `template_id`, `profile_id` or the Happ
reference according to its type. A stale revision returns `409`; missing or
incompatible documents are rejected. Deleting a referenced document is blocked,
including bulk deletion.

Document and reference checks use current locking reads, including on MySQL and
MariaDB sessions with an existing transaction snapshot. A cache/worker notification
failure after commit returns the saved snapshot with a synchronization warning;
it does not report that an already committed write failed.
...
The dashboard regression suite runs with `bun run test:client-settings` from
`dashboard/`. PR checks run it together with a production build. A separate MySQL
CI job exercises concurrent changes against an exclusive test database.

This feature includes native documents and automatic server-group generators in
one workflow. It does not add inactive-user notices or status-based rule matching.
