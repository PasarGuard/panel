...
from app.models.settings import NATIVE_TEMPLATE_BY_TARGET, ConfigFormat, Subscription
...
class ClientWorkspaceOperation(ClientTemplateOperation):
    ...
    async def bound_subscription(self, snapshot, request, template_id):
        ...
        for index in request.bind_rule_indices:
            ...
            elif request.template.template_type in (
                ClientTemplateType.xray_profile,
                ClientTemplateType.singbox_profile,
            ):
                rule.pop("template_id", None)
                rule.pop("happ_routing", None)
                rule["profile_id"] = template_id
            else:
                rule.pop("profile_id", None)
                rule.pop("happ_routing", None)
    ...
    async def apply(self, db, request):
        ...
        try:
            ...
            result.sync_warning = "cache_refresh_failed"
            logger.exception("Workspace saved; local cache refresh failed")
        for topic in (MessageTopic.CLIENT_TEMPLATE, MessageTopic.SETTING):
            try:
                await router.publish(topic, {"action": "refresh"})
            except Exception:
                if result.sync_warning is None:
    ...
    async def preview(self, db, request, admin):
        ...
        for rule in subscription.rules:
            references = [
                ...
                (rule.template_id, NATIVE_TEMPLATE_BY_TARGET.get(rule.target)),
            ]
