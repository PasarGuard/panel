...
EXPLICIT_TEMPLATE_TYPES = {ClientTemplateType.happ_routing}
...
class ClientTemplateOperation(BaseOperation):
    ...
    async def create_client_template(
        ...
        if new_template.template_type in EXPLICIT_TEMPLATE_TYPES and new_template.is_default:
    ...
    async def modify_client_template(
        ...
        if modified_template.content is not None:
            ...
            if ClientTemplateType(db_template.template_type) == ClientTemplateType.happ_routing:
        ...
        if ClientTemplateType(db_template.template_type) in EXPLICIT_TEMPLATE_TYPES and modified_template.is_default:
            await self.raise_error("Explicit templates cannot be set as default", 400)

        if (
            ClientTemplateType(db_template.template_type) not in EXPLICIT_TEMPLATE_TYPES
    ...
    async def _validate_unreferenced(self, db: AsyncSession, template_ids: set[int]) -> None:
        ...
        if referenced_ids:
            ...
            await self.raise_error(
                message=f"Cannot delete client template(s) referenced by settings rules: {formatted_ids}",
                code=409,
                db=db,
            )
