...
class SubscriptionOperation(BaseOperation):
    ...
    ) -> tuple[str | bytes, str, ConfigFormat, None]:
    ...
    async def _rule_template(self, db, template_id: int, overrides=None):
        ...
        if template is None:
            template = await get_client_template_by_id(db, template_id)
        if template is None:
    ...
    async def rule_response_headers(self, db, rule, sub_settings, variables, overrides=None):
    ...
    async def user_subscription(
        ...
        if is_subscription_page_request:
            ...
            conf, media_type, client_type, _ = await self.fetch_rule_config(db, user, matched_rule)
    ...
    async def user_subscription_rule_preview_by_id(
        ...
        if matched_rule.template_id is None and default_source is not None and template_overrides is None:
        ...
        with collect_preview_exceptions() as rendering_exceptions:
            conf, media_type, effective_format, _ = await self.fetch_rule_config(
        ...
        if matched_rule.template_id is not None:
