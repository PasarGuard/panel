...
class User(Base):
    ...
    proxy_settings: Mapped[dict[str, Any]] = mapped_column(JSON(True), server_default=text("'{}'"), default=dict)
    ...
    def become_online(self) -> bool:
        ...
        if self.on_hold_timeout and self.on_hold_timeout.replace(tzinfo=UTC) <= now:
            return True
...
class NextPlan(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, init=False)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    user_template_id: Mapped[int | None] = mapped_column(ForeignKey("user_templates.id"))
...
class ProxyHost(Base):
    ...
    host: Mapped[set[str] | None] = mapped_column(
    ...
    status: Mapped[list[UserStatus] | None] = mapped_column(
...
class Node(Base):
    ...
    core_config_id: Mapped[int | None] = mapped_column(
