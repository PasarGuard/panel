...
async def create_role(db: AsyncSession, data: AdminRoleCreate) -> AdminRole:
    role = AdminRole(
        ...
        disable_users_when_disabled=data.disable_users_when_disabled,
    )
    db.add(role)
    await db.flush()
...
async def modify_role(db: AsyncSession, role: AdminRole, data: AdminRoleModify) -> AdminRole:
    ...
    if data.disable_users_when_disabled is not None:
        role.disable_users_when_disabled = data.disable_users_when_disabled
    await db.flush()
    await db.refresh(role)
    return role
