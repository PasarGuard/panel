...
async def get_jetstream_context(nc: nats.NATS) -> api.JetStreamContext:
    """Get JetStream context from NATS connection. JetStream is always enabled."""
    return nc.jetstream()


async def get_or_create_kv_bucket(js: api.JetStreamContext, bucket_name: str) -> KeyValue | None:
...
async def setup_nats_kv(bucket_name: str) -> tuple[nats.NATS | None, api.JetStreamContext | None, KeyValue | None]:
    """
