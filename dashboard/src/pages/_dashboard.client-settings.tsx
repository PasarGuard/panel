...
import { GeneratorConfigurationEditor } from '@/features/client-settings/components/generator-configuration-editor'
...
type EditorSection = 'routing' | 'dns' | 'pools' | 'delivery'
...
export default function ClientSettingsPage() {
  ...
  const nativeDirtyRef = useRef({ routing: false, dns: false })
  function updateNativeDirty(section: 'routing' | 'dns', value: boolean) {
    nativeDirtyRef.current = { ...nativeDirtyRef.current, [section]: value }
    setNativeDirty(previous => (previous[section] === value ? previous : { ...previous, [section]: value }))
  }
  ...
  const ruleTarget = appRule ? rules[appRule.index]?.target : undefined
  const defaultType =
    ruleTarget === 'xray'
      ? 'xray_subscription'
      : ruleTarget === 'sing_box'
        ? 'singbox_subscription'
        : ruleTarget === 'clash' || ruleTarget === 'clash_meta'
          ? 'clash_subscription'
          : app
            ? NATIVE_TYPES[app.format]
            : undefined
  const currentSource = assignedSource ?? (app ? snapshot?.templates.find(item => item.template_type === defaultType && item.is_default) : undefined)
  ...
  const generator = isGeneratorType(template?.template_type ?? mode)
  const generatorParse = generator && template ? parseSubscriptionProfileContent(template.content) : null
  const sourceError = template ? codeSourceError(template.template_type, template.content) : null
  const documentValid =
    valid &&
    (!generatorParse || generatorParse.success) &&
    !hasNativeDirty &&
    !sourceError &&
    (view === 'code' || !template || template.template_type.includes('user_agent') || (nativeValidity.routing && nativeValidity.dns))
  const dirty = workspaceHasChanges(snapshot, change) || hasNativeDirty
  const labelForType = (type: string) => `${configurationLabel(type)}${isGeneratorType(type) ? ` · ${t('clientSettings.generator', 'Generator')}` : ''}`
  ...
  useEffect(() => {
    if (section === 'pools' && !generator) setSection('routing')
  }, [section, generator])
  ...
  useEffect(() => {
    if (!query.data || query.data.revision === snapshot?.revision) return
    if (dirty || nativeDirtyRef.current.routing || nativeDirtyRef.current.dns) {
      setConflict(true)
      setError(t('clientSettings.conflict', 'Another administrator changed these settings. Your draft is preserved. Copy it before reloading, then apply your changes to the latest version.'))
    } else {
      setSnapshot(query.data)
      // A reverted edit can leave an equal change object with an old revision.
      // Drop it together with its rules when adopting a clean server snapshot.
      setChange(null)
      setConflict(false)
      setError('')
    }
  }, [query.data, snapshot?.revision, dirty, t])
  useEffect(() => {
    setChange(null)
    nativeDirtyRef.current = { routing: false, dns: false }
  ...
  function selectApplicationSource(value: string) {
    ...
    if (value.startsWith('new:')) {
      switchMode(value.slice(4) as ClientTemplateType)
      if (!template) createDraft(value.slice(4) as ClientTemplateType)
      return
    }
    if (dirty && !window.confirm(t('clientSettings.discardConfirm', 'Discard unsaved changes?'))) return
    const source = snapshot.templates.find(item => item.id === Number(value))
    if (source) {
      ...
      setChange({
        ...
        rules: rules.map((rule, index) => (index === appRule.index ? { ...rule, template_id: undefined, profile_id: undefined, happ_routing: undefined } : rule)),
      })
    } else setChange(null)
    setSection('routing')
    setView('visual')
  ...
  function newContent(type: ClientTemplateType) {
    if (isGeneratorType(type) || type === 'user_agent' || type === 'grpc_user_agent') return DEFAULT_TEMPLATE_CONTENT[type]
  ...
  function switchMode(type: ClientTemplateType) {
    ...
    setMode(type)
    if (section === 'pools') setSection('routing')
    setNativeValidity({ routing: true, dns: true })
    setValid(true)
    if (!snapshot) return
    if (!change?.template) {
  ...
  async function save() {
    ...
    try {
      ...
      if (result.sync_warning)
        toast.warning(t('clientSettings.savedSyncWarning', 'Saved; some running processes could not refresh their settings. Check synchronization before relying on updated delivery.'))
              ...
              <div className="min-w-0 space-y-4">
                {app && (
                  <label className="block space-y-1 text-sm">
                    {t('clientSettings.configurationSource', 'Configuration source')}
                    <select
                      className={selectClass}
                      disabled={!canSaveRules || busy}
                      value={change?.template ? `draft:${change.template.template_type}` : (sourceId ?? '')}
                      onChange={event => selectApplicationSource(event.target.value)}
                    >
                      <option value="">{t('clientSettings.currentDefault', 'Current default')}</option>
                      {change?.template && (
                        <option value={`draft:${change.template.template_type}`}>
                          {labelForType(change.template.template_type)} · {t('clientSettings.draftDocument', 'This document draft')}
                        </option>
                      )}
                      {snapshot.templates
                        .filter(item => applicationTypes(app).includes(item.template_type))
                        .map(item => (
                      ...
                      {canCreate &&
                        applicationTypes(app).map(type => (
                          <option key={type} value={`new:${type}`}>
                            {t('clientSettings.configure', 'Configure')} · {labelForType(type)}
                          </option>
                        ))}
                    </select>
                  </label>
                )}
                {generator && (section === 'routing' || section === 'dns') && (
                  <p className="text-muted-foreground text-sm">
                    {t(
                      'clientSettings.generatorHelp',
                      'Set routing and DNS parameters. The generator builds the client configuration using the user’s available servers; customize pools only when needed.',
                    )}
                  </p>
                )}
                {template && !userAgentTemplate && (
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b pb-3">
                    <div className="flex flex-wrap gap-1">
                      {(['routing', 'dns', ...(generator ? ['pools'] : []), ...(app ? ['delivery'] : [])] as EditorSection[]).map(value => (
                        <Button
                          key={value}
                        ...
                        >
                          {t(`clientSettings.${value}`, value === 'routing' ? 'Routing' : value === 'dns' ? 'DNS' : value === 'pools' ? 'Server pools (optional)' : 'Subscription delivery')}
                        </Button>
                      ))}
                    </div>
                    {section !== 'delivery' && section !== 'pools' && (
                      <div className="flex rounded-lg border p-1">
                        <Button size="sm" aria-pressed={view === 'visual'} variant={view === 'visual' ? 'secondary' : 'ghost'} onClick={() => switchView('visual')}>
                          {generator ? t('clientSettings.parameters', 'Parameters') : t('clientSettings.visual', 'Visual')}
                        </Button>
                        <Button size="sm" aria-pressed={view === 'code'} variant={view === 'code' ? 'secondary' : 'ghost'} onClick={() => switchView('code')}>
                          {generator ? t('clientSettings.generatorJson', 'Generator JSON') : t('clientSettings.code', 'Code')}
                        </Button>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-card space-y-3 rounded-xl border p-4">
                    <label className="block space-y-1 text-sm">
                      ...
                      <select className={selectClass} value={change?.template?.template_type ?? mode} onChange={e => switchMode(e.target.value as ClientTemplateType)} disabled={busy}>
                        {[...Object.values(NATIVE_TYPES), 'xray_profile', 'singbox_profile', 'user_agent', 'grpc_user_agent'].map(type => (
                  ...
                  <>
                    ...
                    <div className="min-w-0 space-y-4">
                      <fieldset disabled={!editorEnabled || busy} hidden={Boolean(app && section !== 'delivery')}>
                        ...
                        {!app && !generator && template.template_type !== 'happ_routing' && (
                      ...
                      <fieldset disabled={busy || (section === 'delivery' && app ? !canSaveRules : !editorEnabled)} className="min-w-0">
                        {section === 'delivery' && app ? (
                          ...
                          />
                        ) : generator && section === 'pools' ? (
                          <div className="space-y-4">
                            <p className="text-muted-foreground text-sm">
                              {t(
                                'clientSettings.poolMembershipHelp',
                                'The default pool includes unclassified hosts without extra setup. For additional pools, assign matching pool IDs in Hosts > Subscription profile.',
                              )}
                            </p>
                            {hasPermission(admin, 'hosts', 'read') && (
                              <Link className="text-sm underline" to="/hosts">
                                {t('clientSettings.configurePoolHosts', 'Configure host pool membership')}
                              </Link>
                            )}
                            <SubscriptionProfileEditor
                              core={nativeFormat === 'sing_box' ? 'sing_box' : 'xray'}
                              structuredOnly
                              showIntro={false}
                            ...
                            <div key={`${editorEpoch}-${template.template_type}-${nativeSection}`} hidden={view !== 'visual' || section !== nativeSection}>
                              {generator ? (
                                <GeneratorConfigurationEditor
                                  ...
                                  onChange={editContent}
                                  onDirtyChange={value => updateNativeDirty(nativeSection, value)}
                                />
                              ) : (
                        ...
                        {generatorParse && !generatorParse.success && !sourceError && (
                          <p role="alert" className="text-destructive mt-3 text-sm">
                            {generatorParse.error}
                          </p>
                        )}
                        {sourceError && (
                          <p role="alert" className="text-destructive mt-3 text-sm">
