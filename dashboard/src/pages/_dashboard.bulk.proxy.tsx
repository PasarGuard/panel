...
import { useGetAllGroups, useGetUsers, useGetAdmins, useBulkModifyUsersProxySettings, XTLSFlows, ShadowsocksMethods } from '@/service/api'
  ...
  return (
    <div className="mt-3 flex w-full flex-col space-y-4 lg:space-y-6">
        ...
        <CardContent className="flex flex-col items-center justify-center space-y-6 py-6 lg:py-8">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <Badge variant="outline" className="flex items-center gap-1">
              ...
              {t('bulk.flowMethod', {
                flow: selectedFlow === 'none' || !selectedFlow ? t('none', { defaultValue: 'None' }) : selectedFlow,
                method: selectedMethod || t('none', { defaultValue: 'None' }),
                defaultValue: 'Flow: {{flow}} | Method: {{method}}',
              })}
            </Badge>
            <Badge variant="outline" className="flex items-center gap-1">
          ...
          <Button onClick={handleApply} className="flex items-center gap-2 px-6" disabled={(!hasValidFlow && !hasValidMethod) || mutation.isPending} size="lg">
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('bulk.confirmApplyProxyTitle', { defaultValue: 'Confirm Apply Proxy Settings' })}</AlertDialogTitle>
