...
it('generator bindings replace native sources and keep rules exclusive in both directions', () => {
  const native = { pattern: '^SFA', target: 'sing_box' as const, template_id: 2, response_headers: { 'x-test': 'keep' } }
  const profile = bindRule(native, 'singbox_profile', 3)
  expect(profile).toMatchObject({ profile_id: 3, response_headers: { 'x-test': 'keep' } })
  expect(profile.template_id).toBeUndefined()
  expect(profile.happ_routing).toBeUndefined()
  expect(bindRule(profile, 'singbox_subscription', 4).profile_id).toBeUndefined()
  expect(bindRule(profile, 'happ_routing', 5).profile_id).toBeUndefined()
  expect(templateUsages([profile], 3)).toHaveLength(1)
})
