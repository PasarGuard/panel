...
export interface WorkspaceSnapshot {
  sync_warning?: 'worker_notification_failed' | null
  revision: string
...
export function ruleTemplateId(rule: DraftRule | undefined): number | undefined {
  return rule?.happ_routing?.template_id ?? rule?.template_id ?? undefined
}
export function templateUsages(rules: readonly DraftRule[], id: number) {
  return rules.flatMap((rule, index) => (rule.template_id === id || rule.happ_routing?.template_id === id ? [{ rule, index }] : []))
}
...
export function codeSourceError(type: ClientTemplateType, content: string): SourceError | null {
  ...
  if (/\{[{%#]/.test(content) && type !== 'happ_routing') return null
...
export function compatibleTemplates(templates: readonly ClientTemplateResponse[], target: string) {
  ...
  return templates.filter(item => (format ? item.template_type === NATIVE_TYPES[format] : (target === 'links' || target === 'links_base64') && item.template_type === 'happ_routing'))
}
...
export function bindRule(rule: DraftRule, type: ClientTemplateType, id?: number): DraftRule {
  const next = { ...rule, template_id: undefined, happ_routing: undefined }
