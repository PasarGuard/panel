...
import { bindRule, CLIENT_APPLICATIONS, configurationLabel, compatibleTemplates, isGeneratorType, ruleTemplateId, type DraftRule } from './workspace-model'
          ...
          <label className="space-y-1 text-sm">
            ...
            <select
              ...
              onChange={event => {
                ...
                onChange({ ...rule, target, ...(compatible ? {} : { template_id: undefined, profile_id: undefined, happ_routing: undefined }) })
              }}
            >
              {TARGETS.map(target => (
          ...
          <label className="space-y-1 text-sm">
            ...
            <select
              ...
              onChange={event => {
                ...
                onChange(source ? bindRule(rule, source.template_type, source.id) : { ...rule, template_id: undefined, profile_id: undefined, happ_routing: undefined })
              }}
            >
              ...
              {sources.map(item => (
                <option key={item.id} value={item.id}>
                  ...
                  {isGeneratorType(item.template_type) ? ` · ${t('clientSettings.generator', 'Generator')}` : ''}
                </option>
              ))}
            </select>
          </label>
