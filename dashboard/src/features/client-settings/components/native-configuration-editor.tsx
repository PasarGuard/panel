  ...
  return (
    <div className="space-y-2">
      <label className="grid gap-1.5 text-sm">
        ...
        {multiline ? (
          <Textarea value={draft} onChange={event => changeDraft(event.target.value)} className="min-h-24 font-mono text-xs" />
        ) : (
          <Input value={draft} onChange={event => changeDraft(event.target.value)} />
        )}
      </label>
      {dirty && (
        <div className="flex items-center gap-2">
          ...
          <Button type="button" size="sm" variant="ghost" onClick={() => changeDraft(value)}>
...
export function NativeConfigurationEditor(props: NativeConfigurationEditorProps) {
  ...
  const dirty = (key: string) => {
    if (!dirtyCallbacks.has(key))
      dirtyCallbacks.set(key, value => {
        if (dirtyFieldsRef.current[key] === value) return
        dirtyFieldsRef.current = { ...dirtyFieldsRef.current, [key]: value }
        // Publish in the input event, before a concurrent query update can adopt
        // a new snapshot. Passive effects alone leave a refetch race window.
        onDirtyChangeRef.current?.(Object.values(dirtyFieldsRef.current).some(Boolean))
        setDirtyFields(dirtyFieldsRef.current)
      })
      ...
      <div className="space-y-3 rounded-md border p-4">
        ...
        <label className="grid gap-1 text-sm">
          ...
          <Textarea
            value={values}
            onChange={event => {
              onDirty(editing !== null || !!event.target.value)
      ...
      <div className="space-y-3 rounded-md border p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="grid gap-1 text-sm">
              ...
              <Input
                ...
                disabled={tagLocked}
                onChange={event => {
                  onDirty(editing !== null || !!address || !!event.target.value || !!port)
        ...
        <label className="grid gap-1 text-sm">
          ...
          <Input
            value={address}
            onChange={event => {
              onDirty(editing !== null || !!event.target.value || !!tag || !!port)
        ...
        {modern && (
          <label className="grid gap-1 text-sm">
            ...
            <Input
              value={port}
              onChange={event => {
                onDirty(editing !== null || !!address || !!tag || !!event.target.value)
