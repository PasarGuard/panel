...
function UserModal({ isDialogOpen, onOpenChange, form, editingUser, editingUserId, editingUserData, onSuccessCallback }: UserModalProps) {
  ...
  const lastCycleTrafficResetAtText = React.useMemo(() => {
    return formatMetaDate(editingUserData?.last_cycle_traffic_reset_at)
  }, [editingUserData?.last_cycle_traffic_reset_at, formatMetaDate])
          ...
          <AccordionItem value="meta-details" className="bg-background mt-2 rounded-sm border px-2">
            ...
            <AccordionContent className="pb-2">
              <div className="bg-background space-y-1.5 rounded-md py-2 text-xs">
                ...
                {lastCycleTrafficResetAtText && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-muted-foreground flex items-center gap-1.5">
                      <CalendarClock className="h-3.5 w-3.5" />
                      {t('lastCycleTrafficResetAt', { defaultValue: 'Last scheduled traffic reset' })}
                    </span>
                    <span dir="ltr" className="text-right">
                      {lastCycleTrafficResetAtText}
                    </span>
