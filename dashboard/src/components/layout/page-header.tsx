    ...
    <div dir={dir} className={cn('bg-background/80 sticky top-0 z-20 mx-auto flex w-full flex-row items-start justify-between gap-4 overflow-hidden px-4 py-4 backdrop-blur-md md:pt-6', className)}>
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col gap-y-1">
        <div className="flex min-w-0 items-center gap-2.5">
          <h1 className="truncate text-2xl font-semibold tracking-tight">{t(title, titleDefault ? { defaultValue: titleDefault } : undefined)}</h1>
          <TooltipProvider>
            <Tooltip>
        ...
        {description && <span className="text-muted-foreground text-sm leading-relaxed whitespace-normal">{t(description, descriptionDefault ? { defaultValue: descriptionDefault } : undefined)}</span>}
      </div>
