// Install dashboard dependencies and run `npx playwright install chromium`, or use PLAYWRIGHT_CHANNEL=msedge/chrome.
...
;(async () => {
  ...
  try {
    ...
    await dialog.getByRole('button', { name: 'Camouflage Settings', exact: true }).click()
    const packet = dialog.locator('input[name="noise_settings.xray.0.packet"]')
    assert.equal(await packet.inputValue(), '[0,1,255]')
    await packet.focus()
    await packet.press('Home')
    await packet.press('ArrowRight')
    await packet.press('ArrowRight')
    await packet.press('ArrowRight')
    await packet.pressSequentially(' ')
    assert.equal(await packet.inputValue(), '[0, 1,255]', 'editing must preserve whitespace instead of compacting the array')
    assert.equal(await packet.evaluate(input => input.selectionStart), 4, 'formatting must not move the caret to the end')
    await packet.pressSequentially('2, ')
    assert.equal(await packet.inputValue(), '[0, 2, 1,255]')
    await packet.press('Tab')
    assert.equal(await packet.inputValue(), '[0,2,1,255]', 'format only after leaving the field')
    ...
    assert.deepEqual(payload.noise_settings, { xray: [{ ...host.noise_settings.xray[0], packet: [0, 2, 1, 255] }] })
    console.log('PASS host range edit/validation and save preserving XMux, array noise and zero delay')
    await dialog.waitFor({ state: 'hidden' })
    await page.getByText('Contract test host', { exact: true }).first().click()
    await dialog.getByRole('button', { name: 'Camouflage Settings', exact: true }).click()
    assert.equal(await packet.inputValue(), '[0,1,255]', 'reopening must reflect the loaded host instead of a stale draft')
    await packet.fill('[0, ')
    assert.equal(await packet.inputValue(), '[0, ', 'incomplete array text must remain editable')
    await packet.pressSequentially('1, 42]')
    assert.equal(await packet.inputValue(), '[0, 1, 42]')
    await Promise.all([page.waitForResponse(r => r.url().includes('/api/host/') && r.request().method() === 'PUT'), packet.press('Enter')])
    assert.deepEqual(writes.at(-1).body.noise_settings.xray[0].packet, [0, 1, 42], 'keyboard submission must use the current parsed draft')
    await dialog.waitFor({ state: 'hidden' })
    console.log('PASS array packet caret, blur formatting, reopen and keyboard submission')
    await page.goto(base + '/#/hosts')
